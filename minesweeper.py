...
class MinesweeperAI():
    """Minesweeper game player with probabilistic reasoning"""
    ...
    def calculate_probabilities(self):
        """
        Calculate the probability of each unexplored cell being a mine
        ...
        if unexplored_cells:
            base_probability = len(self.mines) / (self.height * self.width)
        else:
            base_probability = 0
        ...
        # Adjust probabilities based on knowledge sentences
        for sentence in self.knowledge:
            for cell in sentence.cells:
                if cell in probabilities:
                    probabilities[cell] = max(
                        probabilities[cell],
                        sentence.count / len(sentence.cells) if len(sentence.cells) > 0 else 0
                    )
    ...
    def make_random_move(self):
        ...
        Uses probabilities to pick the cell with the lowest likelihood of being a mine.
        """
